...
When you see **`/api/auth/signin?error=OAuthCallback`** and a **503 Service Temporarily Unavailable** from nginx, the OAuth callback from Google is failing. Usually either nginx cannot reach the Next.js app, or the app crashes/times out during the callback.
...
The repo’s `infrastructure/nginx/nginx.conf` now has longer timeouts for `/api/auth/`:

- `proxy_connect_timeout 30s`
- `proxy_send_timeout 60s`
- `proxy_read_timeout 60s`

After pulling the change, reload nginx on the server:
